        ...
        .responseDecodable(of: Login_kakao.self){ response in
            ...
            case.success(let response):
                if response.isSuccess {
                    //code 저장
                    print("카카오로 회원가입 성공")
                    ud.set(response.code, forKey: "joincode")
...
//                viewController.didSuccess_ovalue(response)
    ...
    //카카오 자동로그인 - jwt, userId userdefault로 저장
    func auto_loin(_ viewController : OnboardingViewController){
        
        let params = [  "userId" : ud.string(forKey: "userId"),
                        "jwt" : ud.string(forKey: "jwt")]
        
        AF.request(base_url + "/users/auto-login",
                   method: .post,
        parameters: params,
                   headers: nil)
        .responseDecodable(of: auto_login.self){ response in
            ...
            case.success(let response):
                if response.isSuccess {
                    //code 저장
                    print("post 성공")
                    ud.set(response.code, forKey: "logincode")
